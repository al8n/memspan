  ...
  ($name:ident, $prefix_len:ident, $mask:ident) => {
    ...
    pub(super) unsafe fn $name(input: &[u8]) -> usize {
      ...
      while cur + 2 * CHUNK <= len {
        let c0 = _mm512_loadu_si512(ptr.add(cur) as *const i32);
        let c1 = _mm512_loadu_si512(ptr.add(cur + CHUNK) as *const i32);
        let m0 = $mask(c0);
        let m1 = $mask(c1);
      ...
      while cur + CHUNK <= len {
        let chunk = _mm512_loadu_si512(ptr.add(cur) as *const i32);
        let bits = $mask(chunk);
        if bits != !0u64 {
      ...
      let chunk = _mm512_loadu_si512(ptr.add(overlap_start) as *const i32);
      let bits = $mask(chunk);
