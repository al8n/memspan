...
`memspan` 提供零分配、兼容 `no_std` 的函数，用于在 ASCII 字节类中跳过、计数和定位字节，并在运行时自动分发到最佳可用的 SIMD 后端：
...
`memspan` 采用 MIT 许可证和 Apache License（版本 2.0）双重授权。
...
[en-url]: https://github.com/al8n/memspan/tree/main/README.md
